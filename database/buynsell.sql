...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addItem` (IN `creator_id` INT, IN `title` VARCHAR(128), IN `description` VARCHAR(4096))  READS SQL DATA
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAvailableAds` ()  READS SQL DATA
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getItem` (IN `id` INT)  READS SQL DATA
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `markItemAsSold` (IN `item_id` INT, IN `user_id` INT)  READS SQL DATA
BEGIN
